...
for I=1:size(breath_folder,1)
    ...
    for II = 1:length(laser_onsets)
        ...
        breath_trials(II,:) = breath_zscored;
...
hold on
patch('XData',[0, 0, 60, 60],'YData',[-18, 18, 18, -18],'EdgeColor','none','FaceColor','green','FaceAlpha',0.2);

patch('XData',[xlims fliplr(xlims)],'YData',[mean(breath)+std(breath)/sqrt(trials_num) fliplr(mean(breath)-std(breath)/sqrt(trials_num))],'EdgeColor','none','FaceColor',[193 219 187]./255,'FaceAlpha',0.7);
...
patch('XData',[xlims fliplr(xlims)],'YData',[mean(breath_ctrl)+std(breath_ctrl)/sqrt(ctrl_trials_num) fliplr(mean(breath_ctrl)-std(breath_ctrl)/sqrt(ctrl_trials_num))],'EdgeColor','none','FaceColor',[0.75 0.75 0.75],'FaceAlpha',0.7);
