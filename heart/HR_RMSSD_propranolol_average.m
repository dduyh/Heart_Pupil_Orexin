...
pre_propanolol_heartRate = [];
post_propanolol_heartRate = [];

pre_propanolol_RMSSD = [];
post_propanolol_RMSSD = [];
...
for I=1:size(ECG_folder,1)
    ...
    post_trace_start = vid_start+900*Sample_Rate;
    post_trace_end = post_trace_start+trace_duration*Sample_Rate-1;

    % Raw ECG signals

    pre_ECG_raw = datas(vid_start:pre_trace_end,2)';
    post_ECG_raw = datas(post_trace_start:post_trace_end,2)';
    ...
    pre_propanolol_heartRate = [pre_propanolol_heartRate; mean(1./pre_resting_RR_intervals)];

    pre_propanolol_RMSSD = [pre_propanolol_RMSSD; sqrt(mean((diff(pre_resting_RR_intervals)).^2))];

    post_running = datas(post_trace_start:post_trace_end,1)';
    signedThreshold = 2^(32-1);
    ...
    post_propanolol_heartRate = [post_propanolol_heartRate; mean(1./post_resting_RR_intervals)];

    post_propanolol_RMSSD = [post_propanolol_RMSSD; sqrt(mean((diff(post_resting_RR_intervals)).^2))];
end

%%

heart_rate_data = [pre_propanolol_heartRate; post_propanolol_heartRate];
...
b1(2).BoxFaceColor = '#9392BE';

HR = [pre_propanolol_heartRate post_propanolol_heartRate];
...
[h_HR, p_HR, ci_HR, stats_HR] = ttest(pre_propanolol_heartRate, post_propanolol_heartRate)

%%

heart_rate_variability = [pre_propanolol_RMSSD; post_propanolol_RMSSD].*1000;
...
b2(2).BoxFaceColor = '#9392BE';

HRV = [pre_propanolol_RMSSD post_propanolol_RMSSD].*1000;
...
title({'HRV Before/After Propranolol Injection','',''},'FontSize',24,'FontWeight','bold')
...
[h_HRV, p_HRV, ci_HRV, stats_HRV] = ttest(pre_propanolol_RMSSD, post_propanolol_RMSSD)
