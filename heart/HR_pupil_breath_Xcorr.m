...
breath = breath_data(data_start:data_end);

pixDiff = diff(breath);

[pksVal, pksLocs]=findpeaks([pixDiff(1); pixDiff],FrameRate,'MaxPeakWidth',0.2,'MinPeakHeight',1.5,'Threshold',1e-4,'WidthReference','halfheight');

BB_intervals = diff(pksLocs);
breathRate=1./BB_intervals;

breath_outlier = filloutliers(breathRate,"nearest","percentiles",[0 99]);

breathRate_interp = interp1(pksLocs(2:end)*Sample_Rate,breath_outlier,1:size(datas,1),'nearest','extrap');

breathRate_interp_smooth = movmean(breathRate_interp,[smooth_window*Sample_Rate 0]);
...
heartRate_bpm_outlier = filloutliers(heartRate_bpm,"nearest","percentiles",[1.5 99]);
...
%% Plot pulse, heartRate, breath and pupil size
...
subplot(4,1,2);
plot(time, breathRate_interp_smooth,'color','#479650','LineWidth',2)
xlim([0 time(end)])
title('Breathing Rate','FontSize',15,'FontWeight','bold','color','#479650')
ax = gca;
...
[r,lags] = xcorr(heartRate_bpm_interp-mean(heartRate_bpm_interp),pupil_interp-mean(pupil_interp),'normalized');
figure
stem(lags,r)

[r,lags] = xcorr(pupil_interp-mean(pupil_interp),breathRate_interp-mean(breathRate_interp),'normalized');
figure
stem(lags,r)

[r,lags] = xcorr(heartRate_bpm_interp-mean(heartRate_bpm_interp),breathRate_interp-mean(breathRate_interp),'coeff');
figure
stem(lags,r)
