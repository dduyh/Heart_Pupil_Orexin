...
for I=1:size(folder,1)
    ...
    trace_start = CS_start-120*Sample_Rate;
    ...
    pupil_start = round((step_timepoint(4)-step_timepoint(1))*FrameRate)-120*FrameRate+1;
    pupil_end = pupil_start+trace_duration*FrameRate-1;

    raw_pupil = areas(pupil_start:pupil_end);

    pupil_zscored = (raw_pupil - mean(raw_pupil(1:120*FrameRate))) / std(raw_pupil(1:120*FrameRate));
...
line([120,120],[min(mean(run)-std(run)/sqrt(trials_num)),max(mean(run)+std(run)/sqrt(trials_num))],'Color','k','linestyle','--','LineWidth',2);
line([240,240],[min(mean(run)-std(run)/sqrt(trials_num)),max(mean(run)+std(run)/sqrt(trials_num))],'Color','k','linestyle','--','LineWidth',2);
xlim([0 xlims(end)])
axis off
...
line([120,120],[min(mean(pupil)-std(pupil)/sqrt(trials_num)),max(mean(pupil)+std(pupil)/sqrt(trials_num))],'Color','k','linestyle','--','LineWidth',2);
line([240,240],[min(mean(pupil)-std(pupil)/sqrt(trials_num)),max(mean(pupil)+std(pupil)/sqrt(trials_num))],'Color','k','linestyle','--','LineWidth',2);
...
line([120,120],[min(mean(trials)-std(trials)/sqrt(trials_num)),max(mean(trials)+std(trials)/sqrt(trials_num))],'Color','k','linestyle','--','LineWidth',2);
line([240,240],[min(mean(trials)-std(trials)/sqrt(trials_num)),max(mean(trials)+std(trials)/sqrt(trials_num))],'Color','k','linestyle','--','LineWidth',2);
xlim([0 xlims(end)])
ylim([11.3 12.7])
