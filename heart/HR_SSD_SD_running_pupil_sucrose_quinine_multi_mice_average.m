...
folder = {'m483_L_VTA_R_NAc_oxLight\Apr_15_2024';
    'm486_L_NAc_nLightR\Apr_15_2024';
    'm487_L_VTA_nLightR\Apr_15_2024';
    'm1772\Apr_15_2024';
    'm1773\Apr_15_2024';
    'm1774\Apr_15_2024'};
...
SD = NaN(trials_num,trace_duration);
...
for I=1:size(folder,1)
    ...
    trace_start = ceil(step_timepoint(2)*Sample_Rate)+1020*Sample_Rate+1;
    ...
    heartRate_interp_smooth = movmean(heartRate_bpm_interp,1*Sample_Rate);
    ...
    SSD_interp_smooth = movmean(SSD_interp,1*Sample_Rate);

    RMSSD(I,:) = SSD_interp_smooth;

    % pupil size

    pupil_start = round((step_timepoint(2)-step_timepoint(1))*FrameRate)+1020*FrameRate+1;
...
subplot(5,1,5)
hold on
patch('XData',[pupil_xlims fliplr(pupil_xlims)],'YData',[mean(SD)+std(SD)/sqrt(trials_num) fliplr(mean(SD)-std(SD)/sqrt(trials_num))],'EdgeColor','none','FaceColor',[0.75 0.75 0.75],'FaceAlpha',0.7);
plot(pupil_xlims,mean(SD),'Color','k','LineWidth',2)
xlim([0 pupil_xlims(end)])
ylim([0.002 0.015])
