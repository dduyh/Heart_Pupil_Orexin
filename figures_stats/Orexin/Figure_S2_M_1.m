...
peak_r_RunPup_values = zeros(size(folder,1), 1);
peak_lags_RunPup_values = zeros(size(folder,1), 1);

peak_r_RunHR_values = zeros(size(folder,1), 1);
peak_lags_RunHR_values = zeros(size(folder,1), 1);

peak_r_HRPup_values = zeros(size(folder,1), 1);
peak_lags_HRPup_values = zeros(size(folder,1), 1);

Peak_r_RunPup_values = [];
Peak_lags_RunPup_values = [];

Peak_r_RunHR_values = [];
Peak_lags_RunHR_values = [];

Peak_r_HRPup_values = [];
Peak_lags_HRPup_values = [];
...
for I=1:size(folder,1)
    ...
    for i = 1:length(good_running_onsets)
        ...
        [Peak_r_RunPup_values(k), max_idx] = max(r_RunPup(k,:));
        Peak_lags_RunPup_values(k) = lags(max_idx)/FrameRate;
        ...
        [Peak_r_RunHR_values(k), max_idx] = max(r_RunHR(k,:));
        Peak_lags_RunHR_values(k) = lags(max_idx)/FrameRate;
        ...
        [Peak_r_HRPup_values(k), max_idx] = max(r_HRPup(k,:));
        Peak_lags_HRPup_values(k) = lags(max_idx)/FrameRate;

        k = k+1;

    end

    peak_r_RunPup_values(I) = mean(peak_r_RunPup);
    peak_lags_RunPup_values(I) = mean(peak_lags_RunPup);

    peak_r_RunHR_values(I) = mean(peak_r_RunHR);
    peak_lags_RunHR_values(I) = mean(peak_lags_RunHR);

    peak_r_HRPup_values(I) = mean(peak_r_HRPup);
    peak_lags_HRPup_values(I) = mean(peak_lags_HRPup);

end

%%

[h_RunPup_corr, p_RunPup_corr, ~, stats_RunPup_corr] = ttest(Peak_r_RunPup_values, 0)
[h_RunPup_lag, p_RunPup_lag, ~, stats_RunPup_lag] = ttest(Peak_lags_RunPup_values, 0)

[h_RunHR_corr, p_RunHR_corr, ~, stats_RunHR_corr] = ttest(Peak_r_RunHR_values, 0)
[h_RunHR_lag, p_RunHR_lag, ~, stats_RunHR_lag] = ttest(Peak_lags_RunHR_values, 0)

[h_HRPup_corr, p_HRPup_corr, ~, stats_HRPup_corr] = ttest(Peak_r_HRPup_values, 0)
[h_HRPup_lag, p_HRPup_lag, ~, stats_HRPup_lag] = ttest(Peak_lags_HRPup_values, 0)
...
subplot(3,1,2)
hold on
patch('XData',[xlims fliplr(xlims)],'YData',[mean(pupil)+std(pupil)/sqrt(trials_num) fliplr(mean(pupil)-std(pupil)/sqrt(trials_num))],'EdgeColor','none','FaceColor','#D1E8E6','FaceAlpha',0.5);
plot(xlims,mean(pupil),'Color',[124 190 174]./255,'LineWidth',2)
...
[maxcorr_runpup,l] = max(mean(r_runpup));
plot([xlims(l) xlims(l)],[-0.2 maxcorr_runpup],'--','Color',[124 190 174]./255,'LineWidth',2)
ylim([-0.2 0.8])
...
[maxcorr_runHR,l] = max(mean(r_runHR));
plot([xlims(l) xlims(l)],[-0.1 maxcorr_runHR],'--','Color',[124 190 174]./255,'LineWidth',2)
ylim([-0.1 0.4])
ylabel('Correlation coefficient','FontSize',15,'FontWeight','bold');
xlim([-5 5])
xticks(-5:1:5)
...
[maxcorr_HRpup,l] = max(mean(r_HRpup));
plot([xlims(l) xlims(l)],[-0.1 maxcorr_HRpup],'--','Color',[124 190 174]./255,'LineWidth',2)
ylim([-0.1 0.5])
ylabel('Correlation coefficient','FontSize',15,'FontWeight','bold');
xlim([-5 5])
xticks(-5:1:5)
