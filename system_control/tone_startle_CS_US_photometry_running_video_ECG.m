...
% ECG
ch2 = addAnalogInputChannel(s,'Dev2',3,'Voltage');

% Photometry
ch3 = addAnalogInputChannel(s,'Dev2',19,'Voltage');

ch4 = addAnalogInputChannel(s,'Dev2',0,'Voltage');

% Configure a new listener to process the incoming data.
lh = addlistener(s,'DataAvailable', @(src, event)RunningPlot(src, event, ax1, ax2, ax3, ax4));
...
t.StartDelay = 2400; % 2400 secs = 40 mins
...
function RunningPlot(~, event, ax1, ax2, ax3, ax4)
...
plot(ax4, times(max([0 end-10*1000])+1:end,1), datas(max([0 end-10*1000])+1:end,4), 'b')
ax4.XLim = xlims;
